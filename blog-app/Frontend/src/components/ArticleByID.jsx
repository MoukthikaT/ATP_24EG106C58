...
function ArticleByID() {
  ...
  // delete/restore or block/unblock
  const toggleArticleStatus = async () => {
    const newStatus = !article.isArticleActive;
    const confirmMsg = newStatus ? "Restore this article?" : "Delete/Block this article?";
      ...
      {user?.role === "USER" && (
        <div className={articleActions}>
          <form onSubmit={handleSubmit(addComment)}>
            ...
            <button
              type="submit"
              className="bg-blue-700 text-white px-5 py-2 rounded-lg mt-5"
            >
              Add comment
            </button>
          </form>
